    ...
    if(!(/^\d{4}\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])$/.test(releasedAt.trim())))
        ...
        if (files) {
            ...
            for(const file of files){
                const fileRes = await awsFile.uploadFile(file); 
...
const getBook = async function (req, res) {
    try {
      ...
      for(let i=0; i<keys.length; i++){
        ...
        return res.status(201).send({ status: true, message: "Books Find Successfully", count: findBook1.length, data: findBook1 })
    }
...
try{
    ...
    if(!bookDetails){return res.status(404).send({status:false,msg:" book document does not exist "})}
...
const updateBooks = async function (req, res) {
    ...
    if (!isValidObjectId(bookId)) 
    { return res.status(400).send({status:false,msg:' Invalid Format of userId'})}
    ...
    if (findId.isDeleted === false) {
        ...
        res.status(202).send({ status: true, message: "success", data: updateBook1 })
